...
export default function App() {
  ...
  const pieStats = useMemo(() => {
    ...
    return [...head, { brand: 'Others', total: rest, top1To10: 0, top11To30: 0, top31To50: 0, top51To100: 0, top31To100: rest, percentage: Number((rest / products.length * 100).toFixed(1)) }];
  ...
  const copyStats = async () => {
    const text = [['品牌', '1-10', '11-30', '31-50', '51-100', '31-100', '总数量', '占比'], ...stats.map((row) => [row.brand, row.top1To10, row.top11To30, row.top31To50, row.top51To100, row.top31To100, row.total, `${row.percentage}%`])].map((row) => row.join('\t')).join('\n');
    ...
    <div className="content">
      ...
      {products.length === 0 ? extensionLoading ? <div className="empty"><LoaderCircle className="spin" size={30} /><h2>正在导入 Chrome 扩展采集的数据</h2></div> : <EmptyState /> : <>
        ...
        <section className="data-section"><div className="table-title"><div><span className="eyebrow">BRAND SUMMARY</span><h2>品牌统计表</h2></div><div className="table-actions"><button className="icon-button" title="复制统计结果" onClick={copyStats}><ClipboardCopy size={17} /></button><button className="button compact" onClick={() => exportData('xlsx', products, stats)}><FileSpreadsheet size={16} />导出 Excel</button><button className="button compact" onClick={() => exportData('csv', products, stats)}><Download size={16} />导出 CSV</button><button className="button compact" onClick={() => exportData('json', products, stats)}><Download size={16} />导出 JSON</button></div></div>
          <div className="table-wrap"><table><thead><tr><th>品牌</th><th>1-10 数量</th><th>11-30 数量</th><th>31-50 数量</th><th>51-100 数量</th><th>31-100 数量</th><th>总数量</th><th>占比</th></tr></thead><tbody>{stats.map((row) => <tr key={row.brand}><td className="brand-name">{row.brand}</td><td>{row.top1To10}</td><td>{row.top11To30}</td><td>{row.top31To50}</td><td>{row.top51To100}</td><td className="strong">{row.top31To100}</td><td className="strong">{row.total}</td><td><span className="percent">{row.percentage}%</span></td></tr>)}</tbody></table></div>
        </section>
